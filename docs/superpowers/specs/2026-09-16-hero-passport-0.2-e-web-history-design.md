...
**Status:** implemented and acceptance-qualified; final exact-head PR requalification/review pending  
**Issue:** #46  
...
Add explicit Application/store read contracts for current-Project Quest history and one current-Project Quest detail. Infrastructure performs purpose-built parameterized projections through the existing `SqliteHeroPassportStateStore`; Web maps results into dedicated presentation models.
...
- reuses the already-qualified runtime persistence adapter instead of adding a second read stack;
- no schema change;
...
The selected implementation is the existing `SqliteHeroPassportStateStore` with carefully parameterized `Microsoft.Data.Sqlite` SELECTs. EF Core remains authoritative for schema/migrations, but 0.2-E does not introduce an EF query path beside the store's existing direct runtime SQLite access.

The store first resolves the external workspace fingerprint to the persisted internal ProjectId and display name with a read-only lookup. Subsequent list/detail SQL scopes directly by that internal ProjectId; the internal identifier never crosses Infrastructure.

Requirements:

- use `HeroPassportDatabase.OpenConnectionAsync(...)` and the existing parameterized command helper;
- use a short deferred read transaction for each list/detail use case so multi-SELECT detail/report/Skill reads observe one SQLite snapshot;
- project only fields required by the approved contract;
- no INSERT/UPDATE/DELETE, Project creation, `last seen` update, receipt write, projection rebuild, `ExecuteAsync` mutation helper or commit observer;
- no lazy loading or second persistence abstraction;
- no unbounded child collections;
- report Skills ordered by persisted ordinal and bounded to the canonical 1..3 contract;
- list uses one bounded `ORDER BY ... LIMIT 25` query after Project resolution and must not introduce N+1 per Quest;
- detail uses one Quest/report query plus at most one bounded Skill query after Project resolution;
- no write-upgrade command is permitted inside the deferred transaction.

Microsoft.Data.Sqlite officially supports deferred transactions through `BeginTransaction(deferred: true)`. The transaction can upgrade if a write command is executed, which is precisely why 0.2-E structurally contains SELECT-only commands and separately proves the no-write invariant.
...
For .NET 10 static SSR, canonical not-found detail calls `NavigationManager.NotFound()` and the root `Router` supplies the bounded `NotFoundPage`. Official ASP.NET Core 10 behavior sets HTTP 404 for `NavigationManager.NotFound()` during static SSR. Malformed selectors continue to use the existing cascading `HttpContext` response status path for HTTP 400.
...
- call `GetRuntimeContextAsync` for the established setup gate;
- call Application history methods after setup is complete;
- parse the route QuestId before the history Application/store read;
...
Acceptance must prove that repeated list/detail requests do not change durable state. The qualified implementation uses both a live `PRAGMA data_version` witness and before/after canonical product-table row counts so the assertion covers more than only the tables touched by the happy path.

At minimum the observed product state includes:
...
other canonical product tables present in the schema
```
...
`HeroPassport.Web` process startup already owns database initialization/migration and settings bootstrap required by the application as a whole. The history GET itself must not perform product bootstrap/configuration or create Project/Hero/Quest state.

Product behavior after the Web process has started:

- setup incomplete: follow the existing bounded setup-required Web behavior; the history service checks the current runtime context before its history read;
...
No history read may implicitly configure, create a Hero, create a Project, start/finish a Quest or write progression/receipt state.
...
- no full in-memory table load before the SQL limit;
- Project predicate is applied in SQL;
- ordering and `LIMIT 25` are applied in SQL;
...
Missing-global and foreign-Project canonical QuestIds must use the same bounded NotFound page/body. Error pages/messages must not echo workspace fingerprints, paths, internal IDs, SQL, exception text or foreign-Project existence.
...
- setup-incomplete returns `SetupRequired` before history store access;
- no EF/SQLite/persistence references;
- no hidden mutation forms or POST handlers;
- canonical QuestId parse occurs before history detail Application/store call;
- malformed selector -> 400;
- canonical unavailable selector -> the .NET 10 NotFound flow;
...
3. create Quests in current and foreign Projects;
4. `/history` exposes only current Project rows;
5. >25 current-Project Quests -> exactly newest 25 rendered;
...
10. canonical foreign-Project selector -> byte-for-byte identical bounded 404 body;
11. rendered pages contain no fingerprint/path/request/receipt/hash/database-path leakage;
12. before/after `PRAGMA data_version` and canonical product-table counts prove GET requests did not mutate storage.
...
- the NotFound route is a bounded static-SSR page and does not introduce interactive rendering.
...
- `docs/DEPLOYMENT-MODES.md` only if its route/surface inventory is stale;
- this design status — stop claiming "no implementation yet" once code and acceptance are present.
...
ASP.NET Core 10 Blazor navigation / `NavigationManager.NotFound` / `Router.NotFoundPage`:
https://learn.microsoft.com/en-us/aspnet/core/blazor/fundamentals/navigation?view=aspnetcore-10.0

ASP.NET Core 10 release notes for the .NET 10 NotFound behavior:
https://learn.microsoft.com/en-us/aspnet/core/release-notes/aspnetcore-10.0?view=aspnetcore-10.0

Microsoft.Data.Sqlite transactions and deferred transaction semantics:
https://learn.microsoft.com/en-us/dotnet/standard/data/sqlite/transactions

Microsoft.Data.Sqlite `BeginTransaction(bool deferred)` API for 10.0:
https://learn.microsoft.com/en-us/dotnet/api/microsoft.data.sqlite.sqliteconnection.begintransaction?view=msdata-sqlite-10.0.0
...
Canonical roadmap wording may record the implementation once acceptance evidence exists, but 0.2-E is not declared complete until the exact-head review, merge and post-merge gates above are satisfied.